...
class ValidCommandFlag(val qualifiedName: String) {
    ...
    init {
        if (qualifiedName.startsWith("-"))
            throw IllegalArgumentException("Qualified command flag names should not start with -")
        val trimmed = qualifiedName.trim()
        if (trimmed == "")
            throw IllegalArgumentException("Qualified command flag names should not be whitespace")
        if (trimmed.length <= 1)
            throw IllegalArgumentException("Qualified command flag names should be longer than 1 character")
    }
