...
fun String.isInputInSeconds() : Boolean =
    REGEX_SECONDS.toRegex().matches(this)
fun String.isInputInMinutes() : Boolean =
    REGEX_MINUTES.toRegex().matches(this)
fun String.isInputInHours() : Boolean =
    REGEX_HOURS.toRegex().matches(this)
fun String.isInputAs12HourClock() : Boolean =
    REGEX_12HR_CLOCK.toRegex().matches(this)
fun String.isInputAs24HourClock() : Boolean =
    REGEX_24HR_CLOCK.toRegex().matches(this)

fun String.convert12HTo24H() : String? {
    try {
        // Attempt to match the input string with the regex pattern
        val matchResult = REGEX_12HR_CLOCK.toRegex().find(this)
        ...
        if (matchResult != null) {
            ...
            val isPM = this.endsWith("PM", ignoreCase = true)
...
fun String.convert24HourToTicks() : Long {
    if (!this.isInputAs24HourClock())
        error("\"$this\" is not a valid 24 hour time String")

    val split = this.split(":")
    if (split.size > 2)
        error("\"$this\" split by \":\" resulted in more than two pieces. Should NEVER happen.")
